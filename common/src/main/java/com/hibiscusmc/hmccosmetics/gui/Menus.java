...
public class Menus {
    ...
    private static final HashMap<String, StoreMenu> STORE_MENUS = new HashMap<>();
    ...
    public static void addMenu(@NotNull StoreMenu menu) {
        STORE_MENUS.put(menu.getId().toUpperCase(), menu);
    }
    ...
    @Nullable
    public static StoreMenu getStoreMenu(@NotNull String id) {
        return STORE_MENUS.get(id.toUpperCase());
    }
    ...
    public static boolean hasStoreMenu(@NotNull String id) {
        return STORE_MENUS.containsKey(id.toUpperCase());
    }
        ...
        try (Stream<Path> walkStream = Files.walk(cosmeticFolder.toPath())) {
            walkStream.filter(p -> p.toFile().isFile()).forEach(child -> {
                if (child.toString().endsWith("yml") || child.toString().endsWith("yaml")) {
                    ...
                    try {
                        if(FilenameUtils.removeExtension(child.getFileName().toString()).equalsIgnoreCase("store")) {
                            new StoreMenu(FilenameUtils.removeExtension(child.getFileName().toString()), root);
                        } else {
